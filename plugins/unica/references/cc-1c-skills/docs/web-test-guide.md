...
- Зависимости web-test установлены в package-local scripts каталоге.
...
/web-publish → /web-test → результат
     ↑                          |
     └── правки → v8-runner build/update через MCP ──┘
```
...
Ассистент откроет браузер, перейдёт в раздел и покажет список команд.

```
> Открой список поступлений товаров и покажи первые 10 строк
```

Ассистент откроет список и прочитает таблицу.
...
Ассистент отфильтрует список, откроет найденный элемент двойным кликом и прочитает реквизиты формы.

```
> Открой справочник Контрагенты и найди "Торговый дом"
```

Ассистент может работать с иерархическими справочниками — поиск автоматически сглаживает иерархию.
...
Ассистент откроет форму создания, заполнит шапку и добавит строку в табличную часть.
...
Ассистент заполнит фильтры отчёта по человекочитаемым именам (не надо знать технические имена DCS), нажмёт "Сформировать" и прочитает структурированные данные: заголовки, строки, итоги.
...
Ассистент напишет сценарий, который сформирует отчёт дважды с разными фильтрами и сравнит результаты.
...
Ассистент загрузит расширение через skill `v8-runner` (`operation=build` или `operation=load`), затем через `/web-test` откроет форму и проверит ожидаемое поведение.
...
Ассистент откроет EPF через Ctrl+O, автоматически обработает диалог безопасности (если есть) и прочитает форму.
...
Ассистент будет выполнять команды по одной, показывая состояние формы между шагами.
...
- [Базы данных](db-guide.md) — skill `v8-runner` и MCP `unica.runtime.execute`
