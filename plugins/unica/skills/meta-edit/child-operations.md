...
`lineNumberLength` (алиасы: `line_number_length`, `line-number-length`) принимает
целое значение от 5 до 9 и изменяет только существующий XML-элемент
`LineNumberLength`. Операция разрешена для хранимых объектов, если эффективная
версия совместимости новее `Version8_3_26`: для `DontUse` берётся активная
версия платформенного профиля Unica, для явного `VersionX` — версия `X`.
При эффективной версии `Version8_3_26` и старше значение фиксировано платформой
на 5. Для `Report`, `DataProcessor`, `ExternalReport` и
`ExternalDataProcessor` длина номера строки не ограничена, поэтому свойство
неприменимо. Формат `add-ts` не меняется.

В JSON DSL тот же ключ задаётся в свойствах табличной части:

```json
{
  "modify": {
    "tabularSections": {
      "Товары": { "lineNumberLength": 9 }
    }
  }
}
```
...
Ключи: `name` (rename), `type`, `synonym`, `indexing`, `fillChecking`, `fillValue`, `use` и др.
...
-Operation modify-attribute -Value "Статус: fillValue=Enum.Статусы.EnumValue.Новый"
-Operation modify-ts-attribute -Value "Товары.Статус: fillValue=nil"
-Operation modify-enumValue -Value "СтароеЗначение: name=НовоеЗначение"
```

`fillValue` поддерживается для `modify-attribute` хранимых объектов и для
`modify-ts-attribute` объектов `DataProcessor`/`Report`, когда свойство `FillValue`
присутствует в XML реквизита. `nil` или пустое значение сбрасывает его в
`xsi:nil="true"`. Ссылки вида
`Enum.Имя.EnumValue.Значение` записываются как `xr:DesignTimeRef`; для пустых ссылок
поддержаны виды `Catalog`, `Document`, `ExchangePlan`, `ChartOfAccounts`,
`ChartOfCharacteristicTypes`, `ChartOfCalculationTypes`, `BusinessProcess` и `Task`
в формате `Вид.Имя.EmptyRef`. Bool, decimal и ISO date-time получают соответствующий
`xs:*`, остальные значения записываются как `xs:string`.

Точечная замена сохраняет исходные BOM, EOL, XML declaration, стиль self-closing
элементов и наличие завершающего перевода строки. Недопустимый контекст завершается
ошибкой без записи файла.
