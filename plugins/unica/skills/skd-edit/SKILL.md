...
Когда что: существенная переработка запроса (добавить поля, соединения, переписать пакет) -> выгрузи запрос через `unica.skd.info` с `Mode=query`, `Name=<набор>`, `"Raw": true`, `OutFile=file.sql`; отредактируй файл; верни `set-query @file`. `Raw` отдаёт запрос целиком без декораций, поэтому выгрузка и возврат точны, включая многопакетные запросы. Точечная замена идентификатора или подстроки не требует выгрузки: используй `patch-query`.
