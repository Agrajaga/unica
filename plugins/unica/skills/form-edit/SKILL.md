...
- Удалять можно только публичный элемент, непосредственно принадлежащий контейнеру `ChildItems`. Корневой `AutoCommandBar`, отдельный companion внутри владельца и другие именованные узлы вне такого контейнера защищены; неоднозначные и перекрывающиеся цели также отклоняются.
- До публикации проверяются конфликты с тем же `definition` (`elements`, вложенные `children`/`columns`, `into`, `after`, `elementEvents`) и поддерживаемые ссылки в остающемся XML: binding paths вида `Items.<name>...`, `Form.Item.<name>.StandardCommand.*` и `AdditionSource/Item`.
...
- Весь batch атомарен: планирование, проверки ссылок и итоговая `unica.form.validate` завершаются до фиксации транзакции. При любой ошибке Form.xml не меняется.
...
При preview (`dryRun: true`, значение по умолчанию) файл и cache events не меняются. Apply (`dryRun: false`) публикует только успешно проверенный результат и инвалидирует кэш событием `FormChanged`, только если `changed: true`. Обычный idempotent no-op без удаления сохраняет исходные байты, возвращает `changed: false`, пустой `removed` и не создаёт cache event.
