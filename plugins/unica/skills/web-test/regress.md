...
If no web-client URL exists yet, prepare the local debug contour with the `autonomous-server` skill first. The regression runner consumes an existing URL; it does not publish or manage a web server by itself.
...
**1. Static recon — metadata.** Never invent identifiers. For every metadata object the user mentions, use the matching Unica info surface first: `unica.meta.info` (attributes/tabular sections), `unica.form.info` (form layout), `unica.skd.info` (DCS), `unica.mxl.info` (templates), `unica.role.info` (rights), `unica.subsystem.info` (composition / command interface). If the user names objects you can't find — stop and ask.
